...
def draw_gt_picture(dataset_name: str):
    ...
    plt.xticks([])
    plt.yticks([])
    plt.savefig('./save/gt_map/' + dataset_name + '_gt_map.png', dpi=600)
    plt.show()
...
def parameter_analysis(result_file_path: str, v: str, save_path: str,
                       splitter='\t', loc="lower right", ncol=2):
    ...
    for line in text:
        if line.strip() == '':
            continue
        data.append(line.strip().split(splitter))

    ticks = data[0][1:]
    datasets = []
    oa_max, oa_min = 0, 100
    for i, d in enumerate(data[1:]):
        means_stds = [oa.split('±') for oa in d[1:]]
        # mean_std = np.array(mean_std, dtype='float')
        x, y, yerr = [], [], []
        for j, mean_std in enumerate(means_stds):
            if len(mean_std) == 1:
                continue
            m, s = float(mean_std[0]), float(mean_std[1])
            if m + s > oa_max:
                oa_max = m + s
            if m - s < oa_min:
                oa_min = m - s
        ...
        datasets.append(d[0])
    plt.legend(datasets, loc=loc, prop={"size":14}, ncol=ncol)
    plt.xticks(np.arange(len(ticks)), labels=ticks, fontsize=14)
    plt.yticks(range(int(oa_min // 5 * 5), int((oa_max // 5 + 2) * 5), 5), fontsize=14)
    plt.xlabel(v, size=16)
    plt.ylabel("OA (%)", size=16)
    plt.grid(True)
    plt.savefig(os.path.join(save_path, v + '.png'), dpi=600, bbox_inches='tight', format='png')
    plt.show()
...
def comparison_method_analysis(result_file_path: str, save_path: str):
    ...
    for i, d in enumerate(data[1:]):
        ...
        plt.errorbar(x=np.arange(len(ticks)),
                     y=mean_std[:, 0],
                     yerr=mean_std[:, 1],
                     fmt=symbol[-(len(palette)-len(data[1:])+i+1)] + '--',
                     color=palette[-(len(palette)-len(data[1:])+i+1)],
                     elinewidth=1,
                     capsize=2,
                     ms=6,
                     )
    ...
    plt.xticks(np.arange(len(ticks)), labels=ticks, fontsize=14)
    plt.yticks(range(int(oa_min // 5 * 5), int((oa_max // 5 + 2) * 5), 5), fontsize=14)
    plt.xlabel("Number of training samples per class", size=16)
    plt.ylabel("OA (%)", size=16)
    plt.grid(True)
    dataset_name = result_file_path.split('/')[-1]
    plt.savefig(os.path.join(save_path, dataset_name.split('.')[0] + '.png'), dpi=600, bbox_inches='tight', format='png')
    plt.show()
...
def output_classification_map(model, X_PCAMirror, Y, dataset_name='PU', patch_size=9):
    ...
    plt.xticks([])
    plt.yticks([])
    plt.savefig(os.path.join(save_path, dataset_name + '_background.png'), bbox_inches='tight', dpi=600)
    classification_map[Y.reshape((row, col)) == 0, :] = np.array([255, 255, 255], dtype='uint8')
    plt.figure()
    plt.imshow(classification_map, cmap='jet')
    plt.xticks([])
    plt.yticks([])
    plt.savefig(os.path.join(save_path, dataset_name + '_nobackground.png'), bbox_inches='tight', dpi=600)
...
def calculate_feature_dispersion(dataset_name, feature_map, label):
    ...
    feature_map = tsne.fit_transform(feature_map)
    # x_min, x_max = X_tsne.min(0), X_tsne.max(0)
    # X_norm = (X_tsne - x_min) / (x_max - x_min)
    # feature_map = X_norm
...
if __name__ == '__main__':
    # gt = draw_gt_picture('LongKou')
    # false_color = draw_false_color_picture('LongKou')
    # true_color = draw_true_color_picture('PU')

    file_path = './save/parameter_analysis/DEMAE/pretrained_mask_ratio.txt'
    v = 'Mask ratio'
    # v = 'Input dimension'
    parameter_analysis(result_file_path=file_path, v=v,
                       save_path='./save/parameter_analysis/DEMAE',
                       splitter='\t', loc="lower right", ncol=2)

    # file_path = './save/comparison_method_analysis/DEMAE/LongKou.txt'
    # comparison_method_analysis(result_file_path=file_path, save_path='./save/comparison_method_analysis/DEMAE')
    ...
    #
    # X_PCAMirror, Y, _ = HSI_LazyProcessing(dataset_name=dataset_name, n_pc=dim, no_processing=False,
    #                                                       whiten=True)
    # # X_PCAMirror-Padding
    ...
    # ----- draw class bar ----- #
    # draw_class_bar(dataset_name='GF_HHK', bar_row=3, bar_col=7)
