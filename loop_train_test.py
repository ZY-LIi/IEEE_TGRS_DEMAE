from data import *
import time
import torch
torch.cuda.current_device()
from sklearn.metrics import confusion_matrix, accuracy_score, cohen_kappa_score
import matplotlib.pyplot as plt
from loss_function import *

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')


def get_model(dim, num_class, patch_size, pretrained_weights_path):
    '''
    This function needs to be implemented according to the certain situation.
    :return:
    '''

    from model import DEMAE_finetune
    model = DEMAE_finetune(dim=dim, num_class=num_class, patch_size=patch_size)
    state_dict = torch.load(pretrained_weights_path)
    model.load_state_dict(state_dict, strict=False)

    frozen_modules = [
        model.time_mlp,
    ]
    for module in frozen_modules:
        for param in module.parameters():
            param.requires_grad = False

    return model


def train(model, X_PCAMirror, Y, train_set, dataset_name='PU', patch_size=9, batch_size=64, epoches=120,
          lr=1e-2, shuffle=True, pretrained_timesteps=200, finetuned_timesteps=100, finetuned_mask_ratio=0.75):

    train_loss_list = []

    # ---------- optimizer and loss implementation ---------- #
    loss_func1 = SNREnhanced_FocalLoss(scale=1.0)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, betas=(0.9, 0.999), eps=1e-8, weight_decay=0)
    lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.99)
    # ---------- optimizer and loss implementation ---------- #

    model.train()

    # -------- diffusion betas -------- #
    betas = linear_beta_schedule(timesteps=pretrained_timesteps) if finetuned_timesteps > 0 else 0.
    alphas = 1. - betas
    alphas_cumprod = torch.cumprod(alphas, dim=0) if finetuned_timesteps > 0 else torch.Tensor([1.])
    sqrt_alphas_cumprod = torch.sqrt(alphas_cumprod)
    sqrt_one_minus_alphas_cumprod = torch.sqrt(1. - alphas_cumprod)
    # -------- diffusion betas -------- #

    extra_epoches = 10
    epoches = epoches + extra_epoches
    t = time.time()
    for epoch in range(epoches):

        train_loader = generate_batch(train_set, X_PCAMirror, Y, dataset_name=dataset_name, patch_size=patch_size,
                                      batch_size=batch_size, shuffle=shuffle, augment=True)
        epoch_loss1 = 0
        for step, (x, y) in enumerate(train_loader):

            batch_replication = 3
            x, y = np.repeat(x, batch_replication, axis=0), np.repeat(y, batch_replication, axis=0)

            x, y = torch.Tensor(x), torch.Tensor(y).type(torch.long).squeeze(-1)
            B, C, H, W = x.shape

            num_tokens = H * W
            elements = np.concatenate(
                ([1] * int(num_tokens * finetuned_mask_ratio), [0] * (num_tokens - int(num_tokens * finetuned_mask_ratio) - 1)), axis=0)
            mask = []
            for _ in range(B):
                elements = np.random.permutation(elements)
                mask.append(np.insert(elements, num_tokens // 2, 0))
            mask = torch.Tensor(np.array(mask)).bool()

            x = x.reshape(B, C, -1).transpose(1, 2)
            timestep = torch.randint(0, finetuned_timesteps, (B,)).long() if finetuned_timesteps > 0 else torch.zeros(B).long()
            noise = torch.randn_like(x)
            x_noise = sqrt_alphas_cumprod.gather(-1, timestep).reshape(B, *((1,) * 2)) * x + \
                      sqrt_one_minus_alphas_cumprod.gather(-1, timestep).reshape(B, *((1,) * 2)) * noise
            snr = sqrt_alphas_cumprod.gather(-1, timestep) ** 2 / \
                  sqrt_one_minus_alphas_cumprod.gather(-1, timestep) ** 2

            x, x_noise, y, timestep = \
                x.to(device), x_noise.to(device), y.to(device), timestep.to(device)
            y_hat_cls = model(x_noise.float(), timestep, mask.to(device))

            # ---------- loss back-propagation and parameter optimization ---------- #
            optimizer.zero_grad()
            loss1 = loss_func1(y_hat_cls, y, snr)   # for snr enhanced focalloss
            epoch_loss1 += loss1.item()
            loss = loss1

            loss.backward(retain_graph=True)
            optimizer.step()

        train_loss_list.append([epoch_loss1 / (step + 1)])
        lr_scheduler.step()
        if epoch >= epoches - 3:
            torch.save(model.state_dict(), './save/models/epoch-' + str(epoch) + '.pt')

    train_loss_list = np.array(train_loss_list)
    plt.plot(train_loss_list[:, 0])
    plt.show()

    print('Training model consumes %.2f seconds' % (time.time() - t))


def test(model, X_PCAMirror, Y, test_set, dataset_name='PU', patch_size=9):

    y_pred = []
    y_true = []
    model = model.to(device)
    model.eval()
    t = time.time()
    with torch.no_grad():
        test_loader = generate_batch(test_set, X_PCAMirror, Y,
                                     dataset_name=dataset_name,
                                     patch_size=patch_size, batch_size=1024,
                                     shuffle=False, mode='test')
        for x, y in test_loader:
            x = torch.Tensor(x)
            B, C, H, W = x.shape
            x = x.to(device)
            timesteps = torch.zeros(B).to(device)
            x = x.reshape(B, C, -1).transpose(1, 2)
            y_hat = model(x.float(), timesteps)
            y_pred.extend(y_hat.cpu().argmax(axis=1).tolist())
            y_true.extend(y.reshape(-1).tolist())


    print('Testing model consumes %.2f seconds' % (time.time() - t))

    class_dict = dataset_class_dict.get(dataset_name)
    y_true = [class_dict[i] for i in y_true]
    y_pred = [class_dict[i] for i in y_pred]
    cm = confusion_matrix(y_true, y_pred, labels=class_dict)
    # print(cm)
    oa = accuracy_score(y_true, y_pred)
    mask = np.eye(cm.shape[0])
    acc_for_each_class = np.sum(cm * mask, axis=1) / np.sum(cm, axis=1)
    aa = acc_for_each_class.mean()
    kappa = cohen_kappa_score(y_true, y_pred)
    return cm, oa, aa, kappa, acc_for_each_class


def loop_train_test(dataset_name, run_times, num_PC, train_num, patch_size, batch_size, lr, epoch,
                    pretrained_weights_path, pretrained_timesteps, finetuned_timesteps, finetuned_mask_ratio):

    print('>' * 10, 'Loading Data', '<' * 10)
    X_PCAMirror, Y, [row, col, band] = HSI_LazyProcessing(dataset_name=dataset_name, n_pc=num_PC, no_processing=False,
                                                          whiten=True)

    print('>' * 10, 'Start Experiment', '<' * 10)
    n_class = len(dataset_class_dict.get(dataset_name))

    # X_PCAMirror-Padding
    patch_radius = patch_size // 2
    X_PCAMirror = mirror_concatenate(X_PCAMirror)
    b = default_mirror_width - patch_radius
    X_PCAMirror = X_PCAMirror[b: -b, b: -b]


    metrics = []
    seeds = [1330, 1220, 1336, 1337, 1224, 1236, 1226, 1235, 1233, 1229]
    for seed in seeds:

        seed_dir = os.path.join('./save/result/', dataset_name, str(train_num), str(seed))
        try:
            for file in os.listdir(seed_dir):
                os.remove(os.path.join(seed_dir, file))
        except:
            pass
        if not os.path.exists(seed_dir):
            os.makedirs(seed_dir)

        train_set, train_step, test_set, test_step = \
            split_train_test_set(Y, dataset_name=dataset_name, train_num=train_num, batch_size=batch_size, seed=seed)

        metric = []

        print('Seed: %d' % seed)
        for run in range(run_times):
            for file in os.listdir('./save/models'):
                os.remove(os.path.join('./save/models', file))
            print('Round: %d' % run)
            model = get_model(dim=num_PC, num_class=n_class, patch_size=patch_size,
                              pretrained_weights_path=pretrained_weights_path)
            print('Model Params: ' + str(sum(torch.numel(parameter) for parameter in model.parameters())))
            model = model.to(device)
            train(model, X_PCAMirror, Y, train_set, dataset_name=dataset_name, patch_size=patch_size,
                  batch_size=batch_size, epoches=epoch, lr=lr, pretrained_timesteps=pretrained_timesteps,
                  finetuned_timesteps=finetuned_timesteps, finetuned_mask_ratio=finetuned_mask_ratio)

            print('>' * 10, 'Start Testing', '<' * 10)
            model_path = './save/models'
            model_files = os.listdir(model_path)
            state_dicts = []
            for m in model_files:
                state_dicts.append(torch.load(os.path.join(model_path, m)))

            from model import DEMAE_test
            model = DEMAE_test(dim=num_PC, num_class=n_class, patch_size=patch_size)
            model = model.to(device)

            model.load_state_dict(state_dicts[-1], strict=False)

            cm, oa, aa, kappa, acc_for_each_class = test(model, X_PCAMirror, Y, test_set,
                                                         dataset_name=dataset_name, patch_size=patch_size)
            print('OA: %.4f' % oa)
            print('AA: %.4f' % aa)
            print('Kappa: %.4f' % kappa)
            metric.append([oa, aa, kappa])

            fn = os.path.join('./save/result/', dataset_name, str(train_num), str(seed), 'confusion_matrix_' + str(run) + '.npy')
            np.save(fn, cm)
            fn = os.path.join('./save/result/', dataset_name, str(train_num), str(seed), 'acc_for_each_class_' + str(run) + '.npy')
            np.save(fn, acc_for_each_class)
        metrics.append(metric)
    fn = os.path.join('./save/result/', dataset_name, str(train_num), 'metrics.npy')
    np.save(fn, np.array(metrics))
    metrics = np.array(metrics)
    print(np.mean(metrics, axis=0))


def lazy_test(model, X_PCAMirror, Y, dataset_name='PU', patch_size=9):
    '''
    only output each pixel's feature and then form a feature map
    :param model:
    :param X_PCAMirror:
    :param dataset_name:
    :param patch_size:
    :return:
    '''

    row, col, band = dataset_size_dict.get(dataset_name)
    test_set = np.arange(row * col)
    output_feature_map = []

    # X_PCAMirror-Padding
    patch_radius = patch_size // 2
    X_PCAMirror = mirror_concatenate(X_PCAMirror)
    b = default_mirror_width - patch_radius
    X_PCAMirror = X_PCAMirror[b: -b, b: -b]

    model = model.to(device)
    model.eval()
    with torch.no_grad():
        test_loader = generate_batch(test_set, X_PCAMirror, Y,
                                     dataset_name=dataset_name,
                                     patch_size=patch_size, batch_size=1024,
                                     shuffle=False, mode='test')

        for x, _ in test_loader:
            x = torch.Tensor(x)
            B, C, H, W = x.shape
            x = x.to(device)
            timesteps = torch.zeros(B).to(device)
            x = x.reshape(B, C, -1).transpose(1, 2)
            f = model(x.float(), timesteps)
            f = f.to("cpu").numpy()
            output_feature_map.append(f)

    output_feature_map = np.concatenate(output_feature_map, axis=0)
    print(output_feature_map.shape)
    output_feature_map = output_feature_map.reshape((row, col, -1))
    return output_feature_map


if __name__ == '__main__':

    # lazy test
    model_path = './save/models/'
    dataset_name = 'Houston'
    # model_path = model_path + dataset_name
    n_class = len(dataset_class_dict.get(dataset_name))
    dim = 36
    patch_size = 11
    row, col, band = dataset_size_dict.get(dataset_name)
    model_files = os.listdir(model_path)
    # print(model_files)
    state_dicts = []
    for m in model_files:
        # print(m)
        if m.split('.')[-1] == 'pt':
            print(m + ' is pt file, continue!')
            state_dicts.append(torch.load(os.path.join(model_path, m)))

    X_PCAMirror, Y, _ = HSI_LazyProcessing(dataset_name=dataset_name, no_processing=False, whiten=True, n_pc=dim)
    # X_PCAMirror-Padding
    patch_radius = patch_size // 2
    # X_PCAMirror = mirror_concatenate(X_PCAMirror)
    # b = default_mirror_width - patch_radius
    # X_PCAMirror = X_PCAMirror[b: -b, b: -b]
    # model = Model_EMA(n_class=n_class, state_dicts=state_dicts, raw_dim=band, dim=dim, patch_size=patch_size)
    from model import DEMAE_test

    model = DEMAE_test(dim=dim, num_class=n_class, patch_size=patch_size)
    # state_dict = torch.load('./save/uvit/LongKou_pretrained_weights_patch11_pc36_timesteps300_mask75.pt')
    # print(state_dict)
    model.load_state_dict(state_dicts[-1], strict=False)
    # del model.head  # for SPFormer
    del model.cls_token_classifier

    # correction of forward function is needed!!
    feature_map = lazy_test(model, X_PCAMirror, Y, dataset_name=dataset_name, patch_size=patch_size)
    np.save(os.path.join(model_path, 'feature_map.npy'), feature_map)
